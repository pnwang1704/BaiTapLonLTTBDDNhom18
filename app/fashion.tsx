// app/fashion.tsx – SIÊU PHẨM THỜI TRANG 2025 – NHANH HƠN SHEIN, ĐẸP HƠN ZARA!!!
...
export default function FashionScreen() {
  ...
  const [allProducts, setAllProducts] = useState<any[]>([]); // Dữ liệu gốc từ API
  ...
  // Chỉ gọi API khi đổi sort – tối ưu cực mạnh
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const res = await fetch(`${API_URL}/api/categories/fashion?sort=${sortBy}&limit=100`);
        const data = await res.json();
        setAllProducts(data.products || []);
      } catch (err) {
        console.error('Lỗi load sản phẩm thời trang:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  // TÌM KIẾM SIÊU TỐC TRÊN CLIENT – NHANH HƠN CẢ GOOGLE!!!
  ...
  const handleAddToCart = useCallback((item: any) => {
    ...
    if (stock <= 0) {
      alert('Sản phẩm đã hết hàng rồi ạ!');
      return;
    }
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.loading}>
          ...
          <Text style={styles.loadingText}>Đang tải thời trang hot hit...</Text>
        </View>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={styles.container}>
      {/* Header – Sang trọng như Zara */}
      <View style={styles.header}>
        <TouchableOpacity onPress={() => router.back()}>
          <Ionicons name="chevron-back" size={28} color="#000" />
        </TouchableOpacity>
        <Text style={styles.headerTitle}>Thời Trang</Text>
      ...
      {/* Search Bar – Tìm áo, váy, giày... tức thì */}
      <View style={styles.searchContainer}>
        <Ionicons name="search" size={18} color="#8E8E93" />
        <TextInput
          placeholder="Tìm áo thun, váy, giày sneaker..."
      ...
      <View style={styles.countContainer}>
        <Text style={styles.countText}>
          {searchText
            ? `Tìm thấy ${filteredProducts.length} kết quả cho "${searchText}"`
            : `Tất cả ${allProducts.length} sản phẩm thời trang`}
        </Text>
      </View>
            ...
            {filteredProducts.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {stock <= 5 && stock > 0 && (
                    <Text style={styles.lowStockText}>Chỉ còn {stock} cái!</Text>
                  )}
...
// Styles trắng tinh – giống hệt electronics, fresh, beauty
